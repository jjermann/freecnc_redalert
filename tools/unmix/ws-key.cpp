 ...
 *
 *   Copied directly from Olaf van der Spek's XCC Mixer program
 *   (http://xcc.ra2mods.com). 
 *****************************************************************************/
#include "SDL_types.h"
#include <cstring>
#include "ws-key.h"
...
static void init_bignum(bignum n, Uint32 val, Uint32 len)
{
    memset((void *)n, 0, len * 4);
    n[0] = val;
}

static void move_key_to_big(bignum n, char *key, Uint32 klen, Uint32 blen)
{
    Uint32 sign;
    int i;
...
static void key_to_bignum(bignum n, char *key, Uint32 len)
{
    Uint32 keylen;
    int i;
...
static Uint32 len_bignum(bignum n, Uint32 len)
{
    int i;
    i = len-1;
    while ((i >= 0) && (n[i] == 0))
...
static Uint32 bitlen_bignum(bignum n, Uint32 len)
{
...
static long int cmp_bignum(bignum n1, bignum n2, Uint32 len)
{
    n1 += len-1;
    n2 += len-1;
    while (len > 0) {
...
static void mov_bignum(bignum dest, bignum src, Uint32 len)
{
    memmove(dest, src, len*4);
}

static void shr_bignum(bignum n, Uint32 bits, long int len)
{
    Uint32 i, i2;
...
static void shl_bignum(bignum n, Uint32 bits, Uint32 len)
{
    Uint32 i, i2;
...
static Uint32 sub_bignum(bignum dest, bignum src1, bignum src2, Uint32 carry, Uint32 len)
{
    Uint32 i1, i2;
...
static void inv_bignum(bignum n1, bignum n2, Uint32 len)
{
    bignum n_tmp;
...
static void inc_bignum(bignum n, Uint32 len)
{
    while ((++*n == 0) && (--len > 0))
        n++;
}

static void init_two_dw(bignum n, Uint32 len)
{
...
static void mul_bignum_word(bignum n1, bignum n2, Uint32 mul, Uint32 len)
{
    Uint32 i, tmp;
...
static void mul_bignum(bignum dest, bignum src1, bignum src2, Uint32 len)
{
    Uint32 i;
...
static void not_bignum(bignum n, Uint32 len)
{
    Uint32 i;
    for (i = 0; i < len; i++)
        *(n++) = ~*n;
...
static void neg_bignum(bignum n, Uint32 len)
{
    not_bignum(n, len);
    inc_bignum(n, len);
}

static Uint32 get_mulword(bignum n)
{
    Uint32 i;
    Uint16 *wn;
...
static void dec_bignum(bignum n, Uint32 len)
{
    while ((--*n == 0xffffffff) && (--len > 0))
        n++;
}

static void calc_a_bignum(bignum n1, bignum n2, bignum n3, Uint32 len)
{
...
static void calc_a_key(bignum n1, bignum n2, bignum n3, bignum n4, Uint32 len)
{
    bignum n_tmp;
...
static void process_predata(const Uint8* pre, Uint32 pre_len, Uint8 *buf)
{
    bignum n2, n3;
