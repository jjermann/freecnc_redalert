// mode: -*- C++ -*-
#ifndef STRUCTUREANIMS_H
#define STRUCTUREANIMS_H
...
public:
    RefineAnimEvent(Uint32 p, Structure* str, Uint32 newmoney);
    void anim_func(anim_nfo* data);
    void updateDamaged();
private:
    Structure* str;
...
/// The animation that is shown before a structure is fireing
class ChargeAnimEvent : public BuildingAnimEvent {
public:
    /**
     * @param p the priority of this event
     * @param str pointer to the structure
     */
    ChargeAnimEvent(Uint32 p, Structure* str);
    ~ChargeAnimEvent();
    void anim_func(anim_nfo* data);
private:
    Uint8 frame,framend;
...
/// Repair animation
class RepairAnimEvent : public ActionEvent {
public:
    RepairAnimEvent(Structure* tmpstr);
    ~RepairAnimEvent();
    void run();
private:
    Uint32 ticks;
    ...
    Structure* strct;
    Uint16 maxhealth;
    //How much is repaired for 100 credits?
    Uint16 repairhealth;
    //cost = ticks*100/repairspeed;
    Uint16 repairspeed;
};
